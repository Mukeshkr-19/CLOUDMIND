...
def home():
    ...
    # Emotion logic (Memory/Fear's personality)
    if cpu < 50 and latency < 200:
        mood = "calm 📚"
        message = "All queries are optimized and data looks tidy."
    elif cpu < 80 and latency < 300:
        mood = "busy 🗂️"
        message = "Many lookups in progress, staying steady!"
    else:
        mood = "fragmented 💥"
        message = "Too many reads/writes — indexes overheating!"

    return f"Database (Memory) feels {mood}. CPU={cpu:.1f}%, latency={latency} ms. {message}"
...
def probe():
    if is_stressed:
        ...
        return jsonify({
            ...
            "status": "degraded",
            "is_stressed": True,
            "message": "Database write-lock contention deadlock active"
        }), 503
    return jsonify({
        "service": SERVICE_NAME,
        "status": "ok",
        "is_stressed": False,
        "message": "Database queries healthy"
    }), 200
...
def incident_status():
    return jsonify({
        ...
        "description": "Database write-lock contention deadlock injected" if is_stressed else "All operations normal"
    })
